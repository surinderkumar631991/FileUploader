						...
						for (var i = 0; i < files.length; i++ ) {
							...
							if (status !== true) {
								...
								if (!status[2]) {
									...
									status[1] ? bootbox.alert(status[1], item, l, p, o, s) : null;
								}
                                    ...
                                    if(item) {
                                        if(item.upload && item.upload.status != 'successful') {
                                            ...
                                            if (n.thumbnails.removeConfirmation) {
                                                bootbox.confirm({
                                                    size: "small",
                                                    message: f._assets.textParse(n.captions.removeConfirmation, item),
                                                ...
                                                //bootbox.confirm(f._assets.textParse(n.captions.removeConfirmation, item), c);
                                                //bootbox.confirm(f._assets.textParse(n.captions.removeConfirmation, item), function (result) {
                                                //    if (result) {
	...
	$.fn.fileuploader.defaults = {
        ...
        dialogs: {
            ...
            confirm: function(text, callback) {
                bootbox.confirm(text) ? callback() : null;
            }
        },
        captions: {
